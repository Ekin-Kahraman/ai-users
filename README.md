...
An interactive view of estimated generative AI usage across 147 countries and economies, built from public data. It demonstrates data collection, transparent calculations and browser-based visualisation; the estimates are not official statistics.
...
[`scores.json`](scores.json) records these calculated country scores. They come from the formulas below, not judgements generated by a language model.
...
- The comparable country series in this build starts at H1 2025, so growth is shown for H1 2025 -> H2 2025 -> Q1 2026, not from 2024.
- They do **not** imply that infrastructure causes adoption. The expected-share model is fitted and compared on the same countries, not tested on a held-out sample.
- Readiness weights and the potential-user access ceiling are modelling choices, not validated measures of demand or forecasts.
...
Inspired by [Andrej Karpathy's `karpathy/jobs`](https://github.com/karpathy/jobs): a small static site combining a reproducible data pipeline and an interactive treemap. The attribution records the design inspiration, not an upstream endorsement or a security assessment.
